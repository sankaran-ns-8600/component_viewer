...
;(function($, window, document) {
  ...
  function isSafeResourceUrl(url) {
    ...
    if (u.indexOf('javascript:') === 0 || u.indexOf('vbscript:') === 0) return false;
    if (u.indexOf('http://') === 0 || u.indexOf('https://') === 0 || u.indexOf('blob:') === 0) return true;
    if (u.indexOf('data:') === 0) {
      var after = u.slice(5).split(',')[0];
  ...
  var Overlay = {
    ...
    _isImageItem: false, _isCustomRendered: false,
    ...
    _bindEvents: function() {
      ...
      $(document).on('keydown.cv-overlay', function(e) {
        ...
        /* + / - / = zoom when viewing image (ignore when focus in input/textarea) */
        var tag = (e.target && e.target.tagName) ? e.target.tagName.toLowerCase() : '';
        if (!/(input|textarea|select)/.test(tag) && self._isImageItem && (e.key === '+' || e.key === '=' || e.key === '-')) {
          e.preventDefault();
        ...
        if (e.key === 'Tab' && self.activeInstance.opts.wcag) {
          var shell = self.$shell[0];
          if (!shell) return;
          var focusable = shell.querySelectorAll('button, [href], input:not([disabled]), select, textarea, [tabindex]:not([tabindex="-1"])');
          ...
          var inside = shell.contains(e.target);
    ...
    _buildCarousel: function(inst) {
      ...
      for (var i = 0; i < items.length; i++) {
        (function(idx) {
          ...
          var typeLabel = type === 'pdf' ? 'PDF' : type === 'video' ? 'Video' : type === 'audio' ? 'Audio' : type === 'inline' ? 'Code' : type === 'html' ? 'HTML' : type === 'error' ? '—' : (item.fileExt || type).slice(0, 4);
    ...
    _loadItemCore: function(inst, fadeIn) {
      ...
      /* 5. Poll option (above toolbar) */
      this._updatePollOption(inst, item);

      /* 6. Footer visible if toolbar/zoom or poll option row is shown (not for image error) */
      if (this.$pollOption.hasClass('cv-active') && !(result && result.imageError)) this.$footer.show();

      /* 7. Update prev/next visibility (when loop: false, hide at first/last) */
      this._updateNavButtons(inst);

      /* 8. Preload adjacent images so next/prev to image shows instantly */
      this._preloadAdjacentImages(inst);

      /* 9. Update carousel active state */
      this._updateCarouselSelection(inst);

      /* 10. onOpen */
      if (typeof inst.opts.onOpen === 'function') inst.opts.onOpen(item, this.$stage, inst);

      /* 11. Slideshow — only run when not paused by user */
      if (inst._slideshowTimer) { clearTimeout(inst._slideshowTimer); inst._slideshowTimer = null; }
    ...
    _renderToolbarItems: function($tb, items, inst) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        var $btn = $(
          ...
          '" type="button"' + dataTooltip + ariaLabel + '>' + btnHtml + '</button>'
        );
  ...
  function builtInVideoRenderer(item, $stage, inst) {
    ...
    $jp.jPlayer({
      ...
      play: function()  { isPlaying = true;  $bigPlay.addClass('cv-hidden'); },
      pause: function() { isPlaying = false; $bigPlay.removeClass('cv-hidden'); },
      ended: function() { isPlaying = false; $bigPlay.removeClass('cv-hidden'); },
      supplied: supplied,
  ...
  function builtInPdfRenderer(item, $stage, inst) {
    ...
    var $tbZoomOut = $('<button class="cv-tb-btn"' + tipAttr('zoomOut') + ariaAttr('zoomOut') + '>' + Icons.zoomOut + '</button>');
    $tbZoomOut.on('click', function() { pdfScale = Math.max(0.25, pdfScale - 0.25); renderAllPages(); });
    toolbarItems.push($tbZoomOut[0]);

    var $tbZoomIn = $('<button class="cv-tb-btn"' + tipAttr('zoomIn') + ariaAttr('zoomIn') + '>' + Icons.zoomIn + '</button>');
    ...
    var $tbPrint = $('<button class="cv-tb-btn"' + tipAttr('print') + ariaAttr('print') + '>' + Icons.print + '</button>');
    ...
    _collectItems: function() {
      ...
      this.$container.find(this.opts.selector).each(function() {
        ...
        var defaultItem = {
          ...
          src: $el.data('src') || $el.attr('href') || $el.find('img').attr('src'),
